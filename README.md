...
- **Letsdo::OutputStreamer** — направляет вывод pi по двум потокам: текст
  ...
  заголовок вызова (`⚙ имя: параметры`), результат с отступом, обрезка
  очень большого вывода с заметкой-сводкой, пометка результатов-ошибок
  (`✖ Ошибка: ...`); `finish` гарантирует финальный перевод строки.
