...
module Letsdo
  module Providers
    ...
    #
    # Returns the list of open tasks assigned to the handle (an Array of
    # Letsdo::Providers::Task), or nil when the backlog state is unreadable —
    # the CLI is not on PATH, failed, or its output is not the expected JSON.
    # The loop treats nil as "pause and retry, do not run the agent".
    #
    ...
    class Backlog
      ...
      #
      # @return [Array<Task>, nil] open tasks; nil when the backlog is unreadable;
      #         empty array when there are no open tasks
      def call
